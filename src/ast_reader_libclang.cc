...
bool ASTReaderLibClang::initialize() {
  ...
  filepath =
      "/home/onah/Work/tmp/sample6/tests/sample/build/compile_commands.json";

  std::string ErrorMessage;

  compdb = clang::tooling::JSONCompilationDatabase::loadFromFile(
...
bool ASTReaderLibClang::get_used_locations(Location input, std::vector<Location> &output) {
  ...
  llvm::ArrayRef<std::string> source_paths(input.get_filename());
...
bool ASTReaderLibClang::get_impact_variables(Location input, std::vector<Location> &results) {
  ...
  (void)input;
  (void)results;
  // StatementMatcher SampleMatcher =
